...
const { NODE_ENV, CLIENT_URL = 'http://localhost:3000', ACCESS_TOKEN_SECRET, ACCESS_TOKEN_EXPIRES_IN = 1 * 60, REFRESH_TOKEN_EXPIRES_IN_MONTHS = 3 } = process.env;
...
export const verifyToken = async (req) => {
    const { headers: { cookie = '', authorization = '' }, url } = req;
...
export const generateAccessToken = (res, userID, refreshTokenID) => {
    ...
    res.setHeader('Set-Cookie', `Bearer=${accessToken}; Max-Age=${COOKIE_EXPIRES_IN}; HttpOnly; SameSite=None${isProduction ? '; Secure' : ''}`);
    return accessToken;
};
