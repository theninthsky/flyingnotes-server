...
export const uploadFile = async (ws, message) => {
    ...
    if (!base64)
        return ws.json({ error: 'No file' });
    try {
        if (!validateUploadFile(message))
            throw Error('Invalid parameters');
        const { ops: [file] } = await files.insertOne({
...
export const downloadFile = async (ws, message) => {
    ...
    try {
        if (!validateDownloadFile(message))
            throw Error('Invalid parameters');
